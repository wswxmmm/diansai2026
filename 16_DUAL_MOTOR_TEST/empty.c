...
static void apply_left_speed(int32_t speed)
{
    if (speed > 0) {
        AO_Control(LEFT_FORWARD_DIR, (uint32_t) speed);
    } else if (speed < 0) {
        AO_Control(LEFT_REVERSE_DIR, (uint32_t) -speed);
    } else {
        AO_Stop();
    }
}

static void apply_right_speed(int32_t speed)
{
    if (speed > 0) {
        BO_Control(RIGHT_FORWARD_DIR, (uint32_t) speed);
    } else if (speed < 0) {
        BO_Control(RIGHT_REVERSE_DIR, (uint32_t) -speed);
    } else {
        BO_Stop();
    }
}
...
{
    apply_left_speed(left);
    apply_right_speed(right);
}
...
int main(void)
{
    ...
    last_speed_update = g_milliseconds;
    ...
    while (1) {
        ...
        if ((now - last_speed_update) >= SPEED_UPDATE_MS) {
            uint32_t elapsed_ms = now - last_speed_update;
