...
static uint8_t g_oledAddress;

static bool OLED_I2C_WriteToAddress(
    uint8_t address, const uint8_t *data, uint8_t len)
{
    ...
    DL_I2C_flushControllerTXFIFO(I2C_INST);
    if (DL_I2C_fillControllerTXFIFO(I2C_INST, data, len) != len) {
        DL_I2C_flushControllerTXFIFO(I2C_INST);
        return false;
    }

    DL_I2C_startControllerTransfer(
        I2C_INST, address, DL_I2C_CONTROLLER_DIRECTION_TX, len);
    ...
    status = DL_I2C_getControllerStatus(I2C_INST);
    if ((timeout == 0U) ||
...
{
    return OLED_I2C_WriteToAddress(g_oledAddress, data, len);
}

static bool OLED_WR_Byte(uint8_t data, uint8_t mode)
{
    uint8_t packet[2] = {mode, data};
    return OLED_I2C_Write(packet, 2U);
}
...
bool OLED_Init(void)
{
    uint8_t probe[2] = {OLED_CMD, 0xAEU};

    delay_cycles(CPUCLK_FREQ / 10U);

    g_oledAddress = OLED_I2C_ADDR_PRIMARY;
    if (!OLED_I2C_WriteToAddress(
            g_oledAddress, probe, (uint8_t) sizeof(probe))) {
        g_oledAddress = OLED_I2C_ADDR_SECONDARY;
        if (!OLED_I2C_WriteToAddress(
                g_oledAddress, probe, (uint8_t) sizeof(probe))) {
            g_oledAddress = 0U;
            return false;
    ...
    (void) OLED_WR_Byte(0xAE, OLED_CMD);
