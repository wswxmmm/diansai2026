...
static bool parse_goto(const char *text, uint32_t *sequence,
                       int32_t targetX10[3])
{
    uint8_t i;
    ...
    for (i = 0U; i < 3U; i++) {
        if (!parse_signed(&text, &targetX10[i])) {
            return false;
        }
    }
    while (*text == ' ') {
...
static bool app_joint_angles_in_range(const RobotArmJointAngles *joints)
{
    return (joints->baseDeg >= ARM_BASE_MIN_DEG) &&
           (joints->baseDeg <= ARM_BASE_MAX_DEG) &&
           (joints->lowerDeg >= ARM_LOWER_LINK_MIN_DEG) &&
           (joints->lowerDeg <= ARM_LOWER_LINK_MAX_DEG) &&
           (joints->upperDeg >= ARM_UPPER_LINK_MIN_DEG) &&
           (joints->upperDeg <= ARM_UPPER_LINK_MAX_DEG);
}

static void app_execute_goto(uint32_t sequence, const int32_t targetX10[3])
{
    RobotArmJointAngles target;
    RobotArmIkStatus result;
    float x = (float)targetX10[0] / 10.0f;
    float y = (float)targetX10[1] / 10.0f;
    float z = (float)targetX10[2] / 10.0f;
    bool reached = false;
    uint8_t step;
    if (!g_visionPoseKnown) {
        RpiLink_SendString("ERR GOTO_POSE_UNKNOWN\r\n");
        return;
    }
    if ((z < ARM_VISION_MIN_TOOL_Z_MM) ||
        (z > ARM_VISION_MAX_TOOL_Z_MM)) {
        RpiLink_SendString("ERR GOTO_Z_RANGE\r\n");
        return;
    }

    result = RobotArm_SolveTarget(x, y, z, &g_currentJointAngles, &target);
    if (result != ROBOT_ARM_IK_OK) {
        RpiLink_SendString("ERR GOTO_UNREACHABLE\r\n");
        return;
    }
    if (!app_joint_angles_in_range(&target)) {
        RpiLink_SendString("ERR GOTO_JOINT_RANGE\r\n");
        return;
    }
    ...
    RpiLink_SendString("GOTO SOLVED ");
    RpiLink_SendUnsigned(sequence);
    RpiLink_SendString(" ");
    RpiLink_SendSigned((int32_t)(target.baseDeg * 1000.0f));
    RpiLink_SendString(" ");
    RpiLink_SendSigned((int32_t)(target.lowerDeg * 1000.0f));
    RpiLink_SendString(" ");
    RpiLink_SendSigned((int32_t)(target.upperDeg * 1000.0f));
    RpiLink_SendString("\r\n");

    for (step = 1U; step <= 40U; step++) {
        if (!app_step_toward_joint_angles(
                &target, ARM_IK_TEST_MAX_DELTA_DEG, &reached)) {
            RpiLink_SendString("ERR GOTO_MOTOR_OR_STOP\r\n");
            return;
        }
        RpiLink_SendString("GOTO STEP ");
        RpiLink_SendUnsigned(sequence);
        RpiLink_SendString(" ");
        RpiLink_SendUnsigned(step);
        RpiLink_SendString("\r\n");
        if (reached) {
            RpiLink_SendString("GOTO DONE ");
            RpiLink_SendUnsigned(sequence);
            RpiLink_SendString("\r\n");
    ...
    RpiLink_SendString("ERR GOTO_TOO_MANY_STEPS\r\n");
}
...
static void app_process_rpi_command(char *line)
{
    ...
    uint32_t gotoSequence;
    int32_t gotoTargetX10[3];
    uint8_t i;
    ...
    if (strcmp(line, "PING") == 0) {
        ...
        RpiLink_SendString(
            "STATUS OK ARM_VISION_V10_PHASE_REBASE TRACK=");
        ...
        RpiLink_SendString(
            ...
            "POSE START | RELATIVE START | RELATIVE REBASE | RELATIVE STOP | "
            "JOG seq d1_mdeg d2_mdeg d3_mdeg | "
            "GOTO seq x_x10 y_x10 z_x10 | "
        ...
        RpiLink_SendString("\r\n");
    } else if (strcmp(line, "RELATIVE REBASE") == 0) {
        if (!g_relativeVisualMode) {
            ...
            RpiLink_SendString("RELATIVE REBASED LIMIT_MDEG ");
            RpiLink_SendUnsigned(ARM_RELATIVE_JOG_MAX_MDEG);
            RpiLink_SendString(" TOTAL_MDEG ");
            RpiLink_SendUnsigned(ARM_RELATIVE_TOTAL_MAX_MDEG);
            RpiLink_SendString("\r\n");
        ...
        if (!parse_goto(line + 5, &gotoSequence, gotoTargetX10)) {
            RpiLink_SendString("ERR GOTO_FORMAT\r\n");
        } else {
            app_execute_goto(gotoSequence, gotoTargetX10);
        }
...
int main(void)
{
    ...
    RpiLink_SendString(
        "READY ARM_VISION_V10_PHASE_REBASE TRACK_OFF RELATIVE_OFF\r\n");
